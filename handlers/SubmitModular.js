...
async function submit(body, file, ip) {
    try {
        let output = [];
        ...
        await beatmaphelper.addmap(fileMD5);
        const rawVersion = scoreData[17];
        const OsuVersion = Number(scoreData[17]);
        const beatmapinfo = await beatmaphelper.beatmapInfo(fileMD5);
        ...
        const ranked = await beatmaphelper.isRanked(fileMD5);
        let pp;
        const lb = await usertools.getLeaderBoard(userid, mode);
        let leaderboard;
        if(!await beatmaphelper.isRanked(fileMD5)) throw 'beatmap' 
    } catch (ex) {
        switch (ex) {
            default:
                console.error(ex);
